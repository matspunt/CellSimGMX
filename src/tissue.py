...
class TissueConstruction:
    ...
    def replicate_cell_on_grid(xyz_file, nr_of_cells, offset):
        ...
        for atom, coord in zip(new_atoms, new_coordinates):
            line = f"{atom:<2s}"
            line += ''.join(f"{c:>{max_widths[i]}.{8}f}" for i, c in enumerate(coord))
            line += "\n"
    ...
    @staticmethod
    def replicate_on_grid2(xyz_file, num_cells_x, num_cells_y, num_cells_z, offset):
        """
        Replicates a cell on a grid with a specified number of cells in each direction (x, y, z) and a given offset.

        Args:
            xyz_file (str): The path to the input .xyz file containing the original cell.
            num_cells_x (int): Number of cells in the x direction.
            num_cells_y (int): Number of cells in the y direction.
            num_cells_z (int): Number of cells in the z direction.
            offset (float): The offset in Angstrom defines the distance between CELL centers.
        """
        with open(xyz_file, 'r') as f:
            lines = f.readlines()
        ...
        num_atoms_new = nr_atoms * num_cells_x * num_cells_y * num_cells_z
        ...
        # populate the new .xyz file with the displaced coordinates
        num_atoms_new = nr_atoms * num_cells_x * num_cells_y * num_cells_z
        output_lines = [f"{num_atoms_new}\n",
                        f"Generated by replicating {xyz_file} on a {num_cells_x}x{num_cells_y}x{num_cells_z}.\n"]
        for atom, coord in zip(new_atoms, new_coordinates):
            line = f"{atom:<2s}"
            line += ''.join(f"{c:>{max_widths[i]}.{8}f}" for i, c in enumerate(coord))
            line += "\n"
        ...
        new_xyz = xyz_file.replace(".xyz", f"_{num_cells_x}x{num_cells_y}x{num_cells_z}_grid.xyz")
        with open(new_xyz, 'w') as f:
            f.writelines(output_lines)

        print(f"SUCCESS: An .xyz file with {num_atoms_new} atoms was created: {new_xyz}")
        

    @staticmethod
    def replicate_on_grid_with_matrix(xyz_file, num_cells_x, num_cells_y, num_cells_z, offset, matrix_x, matrix_y):
        """
        Replicates a cell on a grid with a specified number of cells in each direction (x, y, z) and a given offset,
        while creating a monolayer called "matrix" in the z-direction with customizable x, y definitions.

        Args:
            xyz_file (str): The path to the input .xyz file containing the original cell.
            num_cells_x (int): Number of cells in the x direction for cubic packing.
            num_cells_y (int): Number of cells in the y direction for cubic packing.
            num_cells_z (int): Number of cells in the z direction for cubic packing.
            offset (float): The offset in Angstrom defines the distance between CELL centers.
            matrix_x (int): Number of cells in the x direction for the "matrix" monolayer.
            matrix_y (int): Number of cells in the y direction for the "matrix" monolayer.
        """
        with open(xyz_file, 'r') as f:
            lines = f.readlines()
        ...
        # Calculate the offset for cubic packing to center it within the matrix
        offset_x = (matrix_x - num_cells_x) / 2.0
        offset_y = (matrix_y - num_cells_y) / 2.0
            ...
            for j in range(num_cells_y):
                for k in range(num_cells_z):
                    for atom, coord in zip(atoms, coordinates):
                        new_atoms.append(atom)
                        new_coord = [
                            coord[0] + i * offset + offset_x * offset,  # Apply offset_x for x
                            coord[1] + j * offset + offset_y * offset,  # Apply offset_y for y
                            coord[2] + k * offset + offset  # Apply offset in the z-direction
                        ]
                        new_coordinates.append(new_coord)

        num_atoms_cubic = nr_atoms * num_cells_x * num_cells_y * num_cells_z

        # Create the "matrix" monolayer
        matrix_atoms = []
        matrix_coordinates = []
        for i in range(matrix_x):
            for j in range(matrix_y):
                for atom, coord in zip(atoms, coordinates):
                    if atom in ("M1"):
                        atom = "B1"
                    matrix_atoms.append(atom)
                    matrix_coord = [
                        coord[0] + i * offset,
                        coord[1] + j * offset,
                        coord[2]
                    ]
                    matrix_coordinates.append(matrix_coord)

        num_atoms_matrix = nr_atoms * matrix_x * matrix_y

        # Calculate the total number of atoms
        num_atoms_total = num_atoms_cubic + num_atoms_matrix

        # Populate the new .xyz file with the cubic packing and "matrix" monolayer
        output_lines = [f"{num_atoms_total}\n",
                        f"Generated by replicating {xyz_file} on a {matrix_x}x{matrix_y} grid with a {num_cells_x}x{num_cells_y}x{num_cells_z} 'cubic' packing.\n"]
        for atom, coord in zip(new_atoms + matrix_atoms, new_coordinates + matrix_coordinates):
            line = f"{atom:<2s}"
            line += ''.join(f"{c:>19.{8}f}" for c in coord)
            line += "\n"
            output_lines.append(line)

        new_xyz = xyz_file.replace(".xyz", f"_{matrix_x}x{matrix_y}matrix_{num_cells_x}x{num_cells_y}x{num_cells_z}_grid.xyz")
        with open(new_xyz, 'w') as f:
            f.writelines(output_lines)

        print(f"SUCCESS: An .xyz file with {num_atoms_total} atoms was created: {new_xyz}")
...
#TissueConstruction.replicate_cell_on_grid('CELL.xyz', 1000, 40)
TissueConstruction.replicate_on_grid2('CELL.xyz', 4, 4, 5, 40)
...
TissueConstruction.replicate_on_grid_with_matrix('CELL.xyz', 1, 1, 1, 40, 3 , 3)